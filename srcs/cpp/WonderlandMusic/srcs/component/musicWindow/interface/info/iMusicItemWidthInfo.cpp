...
bool IMusicItemWidthInfo::getPosItemWidthPtr( int *&result_width_var_ptr, int &result_index, const QPoint &pos ) const {
	return getPosItemWidthPtr( result_width_var_ptr, result_index, pos.x( ), pos.y( ) );
}
bool IMusicItemWidthInfo::getPosItemWidthPtr( int *&result_width_var_ptr, int &result_index, const int x, const int y ) const {
	if( y < 0 || x < 0 )
		return false;
