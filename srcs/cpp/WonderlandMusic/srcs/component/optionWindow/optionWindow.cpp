...
bool OptionWindow::showOptionPanel( ) {
	if( currentOptionPanelWidget == nullptr )
		return false;
	currentOptionPanelWidget->showPanelBefore( );
	mutex->lock( );
	takeCentralWidget( );
...
bool OptionWindow::showOptionPanel( OptionPanel *option_panel ) {
	...
	if( getOptionPanelIndex( index, option_panel ) == false || option_panel->optionWindow != this )
		return Result_Var_Function_Messag_Ptr_Out_Args( false, this, getOptionPanelIndex, tr( "不存在匹配的面板" ) );
	if( currentOptionPanelWidget )
		if( currentOptionPanelWidget->hidePanelBefore( ) == false )
			return Result_Var_Function_Messag_Ptr_Out_Args( false, currentOptionPanelWidget, hidePanelBefore, tr( "面板调用隐藏功能失败" ) );
	if( option_panel )
		if( option_panel->showPanelBefore( ) == false )
			return Result_Var_Function_Messag_Ptr_Out_Args( false, currentOptionPanelWidget, hidePanelBefore, tr( "面板调用显示功能失败" ) );
	mutex->lock( );
	currentOptionPanelWidget = option_panel;
	takeCentralWidget( );
	auto widget = currentOptionPanelWidget->toWidget( );
