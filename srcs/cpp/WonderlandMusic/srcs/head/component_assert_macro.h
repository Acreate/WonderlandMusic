...
#define Component_Call_Fun_Assert_Ptr( ptr, if_assert ) do{auto var_ptr = ptr; if( var_ptr && var_ptr->if_assert ) break;  Message_Error_Out << ( MessageString( ) << "Component_Assert_Ptr( " << classTypeTools::getTypeName( var_ptr ) << " * ") << #ptr << " = "<< ptr << " ) " << #ptr "->" #if_assert; return false ;}while(false)
