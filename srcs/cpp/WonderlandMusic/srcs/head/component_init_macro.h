...
#define Init_Sub_Component_Ptr( ptr ) if( ptr == nullptr || ptr->initBefore( ) == false || ptr->init( ) == false || ptr->initAfter( ) == false) return ( Message_Error_Out << ( MessageString( ) << "Init_Sub_Component_Ptr( " << classTypeTools::getTypeName( ptr ) << " * ") << #ptr<< " = "<< ptr << " )", false)
